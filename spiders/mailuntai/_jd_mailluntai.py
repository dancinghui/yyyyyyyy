...
class JdMailuntaiSpider(BaseSpider):
    def __init__(self, thread_cnt=10, queue_size=200):
        BaseSpider.__init__(self, 'Jdmailuntai', queue_size=queue_size, thread_cnt=thread_cnt)
        self.parser = JdMailuntaiParser(self)

    def dispatcher(self, q):
        self.add_main_job({'url':MailluntaiGlobleData.url_template, 'mainjob':1, 'page':1, 's':1})
